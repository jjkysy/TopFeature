...
# Note that tasks should be one-directional graph
# So that we can further we calculate their dependencies and so on(if any)
##############################################################################
...
class TaskGraphGenerator:
    ...
    def generate_linear_tasks(cls, n_node: int, n_graph: int) -> List[GraphData]:
        ...
        for i in range(n_graph):
            ...
            linear_graph.append(
                GraphData(graph=linear, name=f"linear_{i}")
            )
        return linear_graph
    ...
    def generate_parallel_tasks(cls, n_node: int, n_graph: int) -> List[GraphData]:
        ...
        for i in range(n_graph):
            ...
            parallel_graph.append(
                GraphData(graph=parallel, name=f"parallel_{i}")
            )
    ...
    def generate_hybrid_tasks(cls, n_node: int, n_graph: int) -> List[GraphData]:
        ...
        for i in range(n_graph):
            ...
            hybrid_graph.append(
                GraphData(graph=hybrid, name=f"hybrid_{i}")
            )
        return hybrid_graph
