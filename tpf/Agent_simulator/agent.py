...
class Agent:
    ...
    def update_boundary(agent: AgentData, new_boundary: Polygon) -> None:
        ...
        if not new_boundary.contains(agent.position):
            agent.position = Agent._generate_random_position_within_polygon(new_boundary)

    @staticmethod
    def update_omega_matrix(omega_matrix: np.ndarray, hits_info: dict, agents: list, increase_factor=0.1, decay_factor=0.1) -> None:
